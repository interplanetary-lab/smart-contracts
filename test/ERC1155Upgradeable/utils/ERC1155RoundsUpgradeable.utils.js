 ...
 *
 * @param instance Instance of the smart contract
 * @param roundId
 * @param tokenId
 * @param supply
 * @param startTime
 ...
 *
 * @param tx The transaction send to mint
 * @param instance Instance of the smart contract
 * @param user
 * @param roundId
 * @param amount
...
const roundMintTests = async (
  ...
  const oldBalance = await instance.balanceOf(user, round.tokenId, tx.receipt.blockNumber - 1);
  ...
  const newBalance = await instance.balanceOf(user, round.tokenId);
  ...
  const newTotalSupply = await instance.totalSupply(round.tokenId);
...
const getTokensFromTransferEvent = (txData) => {
  ...
  txData.logs.map((log) => {
    if (log.event === "Transfer") {
      const { to, from, tokenId, amount } = log.args;
